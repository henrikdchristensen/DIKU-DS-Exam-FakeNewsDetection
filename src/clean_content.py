...
def remove_similar_content_in_start_and_end(df: pd.DataFrame, words_compare: int = 5, min_similar: int = 5, max_iterations: int = -1) -> pd.DataFrame:
    print("Removing similar content in the start and end...")
    while iterations < max_iterations or max_iterations == -1:
        iterations += 1
        if max_iterations != -1:
            print(f"Iteration {iterations} of total {max_iterations}", end="")
        else:
            print(f"Iteration {iterations}, no max set", end="")
