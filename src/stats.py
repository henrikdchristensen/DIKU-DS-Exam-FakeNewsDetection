...
class FakeNewsCorpus(Statistics):
    ...
    def barplot_word_frequency(self, nwords=25):
        ...
        true_words, true_words_cnt = self.stat.sort_frequency(
            self.data[self.data[self.binary_type_label] == False][self.content_label].explode().tolist(), percentage=True)
        fake_words, fake_words_cnt = self.stat.sort_frequency(
            self.data[self.data[self.binary_type_label] == True][self.content_label].explode().tolist(), percentage=True)
    ...
    def boxplot_char_frequency(self):
        ...
        true = self.data[self.data[self.binary_type_label]
                                   == False][self.content_label].apply(lambda x: x.count("!"))
        fake = self.data[self.data[self.binary_type_label]
                                   == True][self.content_label].apply(lambda x: x.count("!"))
        data = {"Reliable": true, "Fake": fake}
    ...
    def plot_type(self):
        ...
        words, words_cnt = self.stat.sort_frequency(
            self.data[self.binary_type_label].tolist(), percentage=True)
    ...
    def barplot_domain_to_label_contribution(self, threshold=0, percentage=True):
        ...
        counts = self.data.groupby([self.domain_label, self.type_label])[
            self.content_label].count().unstack()
        ...
        color_list = [self.types_colors[label] for label in percentages.columns]
    ...
    def plot_average_sentence_length(self):
        ...
        # Plot: average sentence length - with outliers
        true = self.data[self.data[self.binary_type_label] ==
                                   False][self.content_label].apply(self.stat.average_sentence_lengths)
        fake = self.data[self.data[self.binary_type_label] ==
                                   True][self.content_label].apply(self.stat.average_sentence_lengths)
        data = {"Reliable": true, "Fake": fake}
        self.stat.boxplot_data1_vs_data2(data=data, ylabel='avg. sentence length', ax=ax1, showfliers=True, title="with outliers")
        
        # Plot: average sentence length
        self.stat.boxplot_data1_vs_data2(data=data, ylabel='avg. sentence length', ax=ax2, showfliers=False, title="without outliers")
    ...
    def plot_pronouns(self):
        ...
        true = self.data[self.fake_news.data[self.binary_type_label] == False][self.content_label].apply(
            lambda x: sum([x.count(word) for word in self.stat.first_pronouns]))
        fake = self.data[self.data[self.binary_type_label] == True][self.content_label].apply(
            lambda x: sum([x.count(word) for word in self.stat.first_pronouns]))
        data = {"Reliable": true, "Fake": fake}
        self.stat.boxplot_data1_vs_data2(data=data, ylabel='# of 1st pronouns', title='1st pronouns', ax=ax1)

        # Plot 2: Second pronouns
        true = self.data[self.data[self.binary_type_label] == False][self.content_label].apply(
    ...
    def plot_sentence_analysis(self):
        ...
        true = self.data[self.data[self.binary_type_label]
                                   == False][self.sentence_analysis_label].apply(lambda x: x[0])
        fake = self.data[self.data[self.binary_type_label]
                                   == True][self.sentence_analysis_label].apply(lambda x: x[0])
        data = {"Reliable": true, "Fake": fake}
        self.stat.boxplot_data1_vs_data2(data=data, ylabel='polarity score', title='Polarity score', ax=ax1)

        # Plot 2: Subjective score
        true = self.data[self.data[self.binary_type_label]
                                   == False][self.sentence_analysis_label].apply(lambda x: x[1])
        fake = self.data[self.data[self.binary_type_label]
                                   == True][self.sentence_analysis_label].apply(lambda x: x[1])
        data = {"Reliable": true, "Fake": fake}
    ...
    def plot_venn(self, nwords=40):
        ...
        true_words, true_words_cnt = self.stat.sort_frequency(
            self.data[self.data[self.binary_type_label] == False][self.content_label].explode().tolist(), percentage=False)
        fake_words, fake_words_cnt = self.stat.sort_frequency(
            self.data[self.data[self.binary_type_label] == True][self.content_label].explode().tolist(), percentage=False)
...
class Liar(Statistics):
    ...
    def plot_party(self, nparties=25):
        ...
        true_party, true_party_cnt = self.stat.sort_frequency(
            self.data[self.data[self.binary_type_label] == False][self.party_label].fillna('none').explode(), percentage=True)
        fake_party, fake_party_cnt = self.stat.sort_frequency(
            self.data[self.data[self.binary_type_label] == True][self.party_label].fillna('none').explode(), percentage=True)
    ...
    def plot_speaker(self, nspeakers=25):
        ...
        true_speaker, true_speaker_cnt = self.stat.sort_frequency(
            self.data[self.data[self.binary_type_label] == False][self.speaker_label].explode(), percentage=True)
        fake_speaker, fake_speaker_cnt = self.stat.sort_frequency(
            self.data[self.data[self.binary_type_label] == True][self.speaker_label].explode(), percentage=True)
    ...
    def plot_subjects(self, nsubjects=25):
        ...
        subjects, subjects_cnt = self.stat.sort_frequency(
            self.data[self.subjects_label].explode(), percentage=True)
        self.stat.barplot(data=subjects, measure=subjects_cnt, num=nsubjects,
                          label='% of total subjects', color='yellowgreen', ax=ax1)
...
class Statistics_FakeNewsCorpus_vs_Liar(Statistics):
    ...
    def barplot_word_frequency(self, nwords=25):
        ...
        self.stat.barplot(data=words_fake_news, measure=words_cnt_fake_news, num=nwords, minmax=(
            0, max_value), label='% of total words', title='FakeNewsCorpus', color='plum', ax=ax1)
        self.stat.barplot(data=words_liar, measure=words_cnt_liar, num=nwords, minmax=(
            0, max_value), label='% of total words', title='LIAR', color='yellowgreen', ax=ax2)
    ...
    def plot_type(self):
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(7, 5))
        self.fake_news.barplot_type(ax=ax1, title="FakeNewsCorpus")
        self.liar.barplot_type(ax=ax2, title="LIAR")
