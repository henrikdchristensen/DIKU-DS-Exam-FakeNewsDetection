...
def main():
    ...
    pd.set_option('display.max_colwidth', None)
    cleaned = pp.clean_text(content)
    tokenized = pp.tokenize_text(cleaned)
    without_stopwords_nltk = tokenized.applymap(
        lambda x: pp.remove_stopwords_nltk(x))

    stop_words_tail = pp.Exploration.get_stopwords(tokenized, 0, 2e-3)
    stop_words_notail = pp.Exploration.get_stopwords(tokenized, 3e-5, 2e-3)
    without_stopwords_freq_tail = tokenized.applymap(
        lambda x: pp.remove_stopwords_freq(x, stop_words_tail))
    without_stopwords_freq_notail = tokenized.applymap(
        lambda x: pp.remove_stopwords_freq(x, stop_words_notail))

    stemmed = without_stopwords_nltk.applymap(lambda x: pp.stem(x))
    pipeline = pipeline.assign(content=content, cleaned=cleaned, tokenized=tokenized,
                               without_stopwords=without_stopwords_nltk, stemmed=stemmed)
...
# Compute the reduction rate of the vocabulary size after stemming.
    ...
    word_count_with_stopwords = pp.Exploration.get_word_info(tokenized)
    word_count_without_stopwords = pp.Exploration.get_word_info(
        without_stopwords_nltk)
    word_count_without_stopwords_freq_tail = pp.Exploration.get_word_info(
        without_stopwords_freq_tail)
    word_count_without_stopwords_freq_notail = pp.Exploration.get_word_info(
        without_stopwords_freq_notail)
    ...
    print(
        f"vocab after freq stopwords tail: {len(word_count_without_stopwords_freq_tail)}")
    print(
        f"vocab after freq stopwords no tail: {len(word_count_without_stopwords_freq_notail)}")

    word_count_after_stemming = pp.Exploration.get_word_info(stemmed)
    print(f"wc after stemming: {len(word_count_after_stemming)}")

    reduction_rate_stopwords = 1 - \
        len(word_count_without_stopwords)/len(word_count_with_stopwords)
    ...
    reduction_rate_stemming = 1 - \
        len(word_count_after_stemming)/len(word_count_without_stopwords)
