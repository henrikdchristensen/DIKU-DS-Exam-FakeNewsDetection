...
def vectorize(to_csv=False):
    ...
    for label, file in files:
        ...
        if to_csv:
            content_tfidf_df = pd.DataFrame(
                content_tfidf.todense(), columns=vect.get_feature_names_out())
            content_tfidf_df.to_csv(
                "../datasets/sample/" + label + "_tfidf.csv", index=False)
...
def k_neighbors_classifier(data):
    ...
    print("k_nearest accuracy:", accuracy_score(
        data["y_test"].astype(int), k_nearest_pred))
...
def support_vector_classifier(data):
    ...
    # Evaluate performance
    print("svc accuracy:" +
          str(accuracy_score(data["y_test"].astype(int), svc_pred)))


def distilBERT():
    content = pp.apply_pipeline(
        "../datasets/sample/train.csv", [], get_batch=True, batch_size=BATCH_SIZE)['content']

    # Tokenizer and model input
    pretrained_weights = 'distilbert-base-uncased'
    tokenizer = ppb.DistilBertTokenizer.from_pretrained(pretrained_weights)
    model = ppb.DistilBertModel.from_pretrained(
        pretrained_weights, from_tf=True)

    # Tokenize input
    tokenized = content.apply(
    ...
    # concatenate all batch outputs back into one tensor
...
def word_embeddings_model(word_embeddings):
    ...
    labels = pp.apply_pipeline(
        "../datasets/sample/train.csv", [], get_batch=True, batch_size=BATCH_SIZE)['type']
    X_train, X_test, y_train, y_test = train_test_split(
        word_embeddings, labels, test_size=0.2, random_state=42)
    print("b")
    layers = [1, 2, 3, 4, 5]
